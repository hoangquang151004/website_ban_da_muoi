# Task 01 — Seed Database & Cấu hình môi trường

**Ước tính:** 20 phút  
**Bắt buộc hoàn thành trước tất cả task còn lại**

---

## Mục tiêu

- Tạo file `.env.local` cho frontend để biết URL backend
- Đảm bảo DB có đủ dữ liệu mẫu để frontend hiển thị

---

## Bước 1 — Tạo file `.env.local` cho frontend

Tạo file mới tại `frontend/.env.local`:

```
NEXT_PUBLIC_API_URL=http://localhost:8000/api/v1
```

> File này đã bị gitignore, không bị commit lên git. Mỗi máy dev cần tạo 1 lần.

**Kiểm tra:** Restart `npm run dev`, mở `http://localhost:3000` — không còn lỗi CORS hay lỗi kết nối API.

---

## Bước 2 — Chạy Alembic migration
...
**Kiểm tra:** Mở MySQL Workbench, xác nhận các bảng đã được tạo:
`users`, `categories`, `uses`, `products`, `product_uses`, `orders`, `order_items`, `reviews`, `stock_logs`

---

## Bước 3 — Tạo script seed dữ liệu mẫu

Tạo file `backend/scripts/seed_sample_data.py`:
...
- [ ] File `frontend/.env.local` tồn tại với `NEXT_PUBLIC_API_URL`
- [ ] `GET http://localhost:8000/health` trả về `{"status": "ok"}`
- [ ] `GET http://localhost:8000/api/v1/products` trả về products từ DB (không phải mảng rỗng)
- [ ] `GET http://localhost:8000/api/v1/categories` trả về danh mục
- [ ] Login bằng `admin@damuoi.com` / `admin123` thành công
